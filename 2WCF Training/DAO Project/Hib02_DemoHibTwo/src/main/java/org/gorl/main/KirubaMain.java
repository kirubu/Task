...
public class KirubaMain {
	...
	{	
		...
		//-----select-------using HQL-----where condition
		/*String wherQuery = "select k.name,k.gender from KirubaObj k where kid>100";
		Query whereQuery = session.createQuery(wherQuery);
		List<Object[]> liWhere = whereQuery.list();
		for(Object[] ar:liWhere)
			System.out.println(ar[0]+" "+ar[1]);*/
		
		//-----select-------using HQL--------all column
		/*String query1 = "from KirubaObj";//from className
		
		//-----select-------using HQL--------multiple column
		String query = "select k.kid, k.name,k.gender from KirubaObj k";
		
		//-----select-------using HQL--------single column
